...
GifImage::Frame::Builder* GifImage::Parser::GetFrameBuilder() {
  if (!active_frame_builder_)
    active_frame_builder_.reset(new Frame::Builder());
...
Result GifImage::Parser::Parse() {
  if (header_only_ && header_complete_)
    return Result::Finish(Result::Code::kOk);
...
Result GifImage::Parser::ParseBlockType() {
  ...
  if (!result.ok()) {
    // End of block is valid EOF.
    return Result::FromIoResult(result, true);
  }
...
Result GifImage::Parser::ParseExtensionType() {
  ...
  switch (*data) {
    ...
    // headers
    // (as 12 and 11 bytes, respectively). Because we ignore the plain text
    // extension entirely
    // and sanity-check the actual length of the application extension header
    // before reading it,
    // we allow GIFs to deviate from these values in either direction. This is
    // important for
    // real-world compatibility, as GIFs in the wild exist with application
    // extension headers
...
Result GifImage::Parser::ParseControlExtension() {
  ...
  // buffer
  // contains at least this many bytes. If the GIF specifies a different length,
  // we
  // allow that, so long as it's larger; the additional data will simply be
  // ignored.
...
Result GifImage::Parser::ParseLocalColorTable() {
  auto result = BuildColorTable(GetFrameBuilder()->GetLocalColorTable());
  if (!result.ok())
    return result;
...
Result GifImage::Parser::ReadLZWData() {
  while (remaining_block_length_ > 0) {
    ...
    auto result = GetFrameBuilder()->ProcessImageData(data, io_result.n());
    if (!result.ok())
      return result;
...
GifImage::Frame::Builder::Builder() {
  frame_ = base::make_unique<Frame>();
}

GifImage::Frame::Builder::~Builder() {}

void GifImage::Frame::Builder::SetGlobalColorTable(
    const ColorTable* global_color_table) {
  frame_->global_color_table_ = global_color_table;
}

void GifImage::Frame::Builder::SetFrameGeometry(uint16_t x_offset,
                                                uint16_t y_offset,
                                                uint16_t width,
                                                uint16_t height) {
...
void GifImage::Frame::Builder::SetProgressive(bool progressive) {
  frame_->is_progressive_ = progressive;
}

void GifImage::Frame::Builder::SetTransparentPixel(size_t value) {
  frame_->transparent_pixel_ = value;
}

void GifImage::Frame::Builder::SetDuration(size_t duration) {
  frame_->duration_ = duration;
}

void GifImage::Frame::Builder::SetDisposalMethod(DisposalMethod method) {
  frame_->disposal_method_ = method;
}

bool GifImage::Frame::Builder::InitDecoder(uint8_t minimum_code_size) {
  lzw_reader_ = base::make_unique<LZWReader>();
  return lzw_reader_->Init(minimum_code_size, 4096,
                           std::function<bool(uint8_t*, size_t)>());
}

Result GifImage::Frame::Builder::ProcessImageData(uint8_t* data, size_t size) {
  return Result::Ok();
}

void GifImage::Frame::Builder::CreateLocalColorTable(size_t size) {
  frame_->local_color_table_.reset(new ColorTable(size));
}

GifImage::ColorTable* GifImage::Frame::Builder::GetLocalColorTable() {
  return frame_->local_color_table_.get();
}

std::unique_ptr<GifImage::Frame> GifImage::Frame::Builder::Build() {
